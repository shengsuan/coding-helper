...
PKG_URL="https://lf3-static.bytednsdoc.com/obj/eden-cn/ylwslo-yrh/ljhwZthlaukjlkulzlp/coohu-coding-helper-1.2.6.tgz"
TMP_FILE="$(mktemp /tmp/coding-helper-XXXXXX.tgz)"

cleanup() { rm -f "$TMP_FILE"; }
trap cleanup EXIT

echo "Downloading coding-helper..."
curl -fsSL -o "$TMP_FILE" "$PKG_URL"

echo "Removing old version (if any)..."
npm uninstall -g @coohu/coding-helper 2>/dev/null || true

echo "Installing coding-helper..."
npm install "$TMP_FILE" -g --registry https://registry.npmjs.org

echo "Done! Run 'coding-helper --version' to verify."
